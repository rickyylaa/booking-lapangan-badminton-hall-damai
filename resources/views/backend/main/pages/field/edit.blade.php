    ...
    <div class="row">
        <div class="col-12 mb-4 mb-sm-5">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <button type="button" class="btn btn-dark w-100px rounded-0" data-bs-toggle="modal" data-bs-target="#fieldModal">VIEW</button>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-12">
            <div class="card border rounded-0 h-100">
                <div class="card-body">
                    <form method="POST" action="{{ route('field.update', $field->id) }}" enctype="multipart/form-data">
                        @csrf @method('PUT')
                        <div class="row px-xxl-2">
                            <div class="col-md-6">
                                ...
                                <div class="form-border-bottom form-control-transparent h6">
                                    <input type="text" name="title" id="title" class="form-control" value="{{ old('title', $field->title) }}" required>
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-border-bottom form-control-transparent">
                                    <select name="status" id="status" class="form-control" required>
                                        <option value="1" {{ old('status', $field->status) == 1 ? 'selected' : '' }}>Active</option>
                                        <option value="0" {{ old('status', $field->status) == 0 ? 'selected' : '' }}>Inactive</option>
                                    </select>
                                </div>
                            ...
                            <div class="col-12">
                                ...
                                <div class="form-border-bottom form-control-transparent h6">
                                    <input type="text" name="price" id="price" class="form-control" value="{{ old('price', $field->price) }}" required>
                                </div>
                                <p class="text-danger small">{{ $errors->first('price') }}</p>
                            </div>
                            <div class="col-12">
                                <label for="image" class="small fw-medium h6 mb-3">UPLOAD IMAGE
                            ...
                            <div class="col-12">
                                ...
                                <div class="form-border-bottom form-control-transparent h6">
                                    <textarea name="description" id="description" class="form-control" rows="3" required>{{ old('description', $field->description) }}</textarea>
                                </div>
                                <p class="text-danger small">{{ $errors->first('description') }}</p>
                            </div>
                        </div>
                        <div class="d-flex justify-content-center align-items-center mb-0">
                            <a href="{{ route('field.index') }}" class="btn btn-secondary w-50 rounded-0 mb-0 me-3">BACK</a>
                ...
                <div class="row px-xxl-2">
                    <div class="col-12">
                        <div class="card bg-transparent text-center p-1 w-100">
                            <img src="{{ asset('storage/fields/'. $field->image) }}" alt="field" class="rounded-0">
                        </div>
                    </div>
                </div>
            </div>
